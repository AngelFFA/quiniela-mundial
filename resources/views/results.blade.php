    ...
    <div class="mx-auto max-w-7xl">
        ...
        <form method="POST" action="{{ route('results.store') }}" class="mt-8 sm:mt-10">
            ...
            <div class="rounded-[2rem] bg-white p-4 shadow-2xl sm:p-7">
                ...
                <div class="mt-8">
                    <h2 class="mb-5 text-2xl font-black text-[#080f2f]">Octavos</h2>
                    <div class="grid gap-4 lg:grid-cols-2">
                ...
                <div class="mt-8">
                    <h2 class="mb-5 text-2xl font-black text-[#080f2f]">Cuartos</h2>
                    <div class="grid gap-4 lg:grid-cols-2">
                ...
                <div class="mt-8">
                    <h2 class="mb-5 text-2xl font-black text-[#080f2f]">Final y tercer lugar</h2>
                    <div class="grid gap-4 lg:grid-cols-2">
                        @foreach($round2Slots as $slot)
                            @php $match = $slot->match; @endphp
                            <div class="rounded-3xl bg-[#f4f6ff] p-4 sm:p-5 {{ $match ? '' : 'opacity-65' }}">
                                <p class="mb-4 text-center text-xs font-black uppercase tracking-[0.14em] text-[#1238ff]">{{ $slot->title }}</p>
                                @if($match)
                                    <div class="grid grid-cols-[1fr_auto_1fr] items-center gap-3">
                                        <div class="text-center">
                                            <img src="https://flagcdn.com/w80/{{ strtolower($match->homeTeam->flag) }}.png" class="mx-auto h-8 w-11 rounded object-cover" alt="">
...
document.addEventListener('DOMContentLoaded',()=>{
 const update=id=>{const a=[...document.querySelectorAll(`.knockout-result[data-match="${id}"]`)];const w=document.getElementById(`ko-winner-${id}`);if(!w||a.length!==2)return;const both=a.every(x=>x.value!=='');w.classList.toggle('hidden',!(both&&Number(a[0].value)===Number(a[1].value)));};
 document.querySelectorAll('.knockout-result').forEach(x=>{update(x.dataset.match);x.addEventListener('input',()=>update(x.dataset.match));});
});
</script>
@endsection
